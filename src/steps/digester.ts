...
export interface DigestPolicy {
  ...
  never?: string[];
  /** Never digest errors — the driver needs the exact failure text. */
  keepErrorsVerbatim: boolean;
  /**
   * Most chunks to summarize when the input does not fit the digester's window.
   *
   ...
   *
   * But *which* cost depends on where the digester runs, so the default does
   ...
   * a large log. A cloud digester is billed per chunk, and a digester that
...
export const DEFAULT_DIGEST_POLICY: DigestPolicy = {
  ...
  // Left unset: resolved per call from where the digester actually runs.
};
 ...
 * 200k raw tokens in the driver's context — it is competing against an 8KB
 ...
 *
 * A *local* digester has no such problem: the marginal cost is wall-clock on
...
/**
 * The digester compresses bulky tool output on the local machine before it
 * ever reaches the cloud driver's context.
 *
 ...
 *     not a deletion. Anything the driver needs can be re-fetched, and a human
 ...
 *     output the driver has already seen would rewrite its prompt prefix and
 ...
 *     the driver's next render, which is the only moment substitution is free.
 */
export async function digest(args: {
  ...
  if (!args.router.has("digester")) {
    return { digested: false, originalTokens, skipReason: "no digester bound" };
  }

  const channel = args.transcript.newSideChannel("digest");

  // Respect the digester's context window.
  //
  // Without this the digester sends the whole result to whatever model is
  ...
  const port = args.router.portFor("digester");
  ...
  // A cloud digester facing a result too large for one call: don't. Spill has
  // already bounded what the driver sees, so paying several full-price calls
  // buys the difference between a preview and a summary — which does not repay
  // within a normal session. Bind a local digester, or pin `maxChunks` to say
  // you meant it.
  if (chunks.length > maxChunks && port.info.locality === "cloud" && policy.maxChunks === undefined) {
    return {
      ...
      skipReason:
        ...
        `what the driver sees, so chunked cloud digestion would cost more than it saves. ` +
        `Bind a local digester, or set maxChunks explicitly.`,
    };
  }
  ...
  // call and gained nothing, and the driver now reads a paraphrase instead of
  // the truth. Keep the original.
  ...
  return { digested: true, text: result.text, servedBy, originalTokens, digestTokens };
}
...
async function runDigest(
  ...
  args.ledger.record("digester", port.info, result);
  args.transcript.assistant(result.text, [], `digester@${port.info.id}`, channel);
  return { text: result.text, servedBy: port.info.id };
}
